...
Rbbt covers several functionalities, some will work right away, some require
to install dependencies or download and process data from databases. Since not
...
when using new parts of the API. In the installation section we try to list
...
1. Do <tt>rbbt_config install organisms</tt> to do deploy the configuration files, this needs to be done just once. 
2. You will need the Entrez gene_info files which lists identifiers, install it by doing <tt>rbbt_config install entrez</tt> 
3. Now you may do <tt>rbbt_config update organisms</tt> toprocess all the organisms, or <tt>rbbt_config update organisms --organism sgd</tt> to process only yeast (sgd).
...
1. Compile and install CRF++[http://crfpp.sourceforge.net/] <tt>rbbt_config install crf++</tt>.
2. Install the Biocreative data used to train the model <tt>rbbt_config install biocreative</tt>
3. Install and update the ner module for that organism <tt>rbbt_config install ner</tt> and <tt> rbbt_config update ner --organism sgd</tt>. You need to have the gems ParseTree and ruby2ruby for this to work. This process can take a long time.

Or, if you wan to use Abner or Banner:

1. Download and install the packages <tt>rbbt_config install abner</tt> or <tt>rbbt_config install banner</tt>
2. Make sure you have the rjb[rjb.rubyforge.org/] gem installed
...
This is the complete list of gem requirements: <tt>ParseTree ruby2ruby simpleconsole rjb rsruby stemmer rand rake</tt>. Some of these gems to not work with ruby 1.9 at the time, or may be a bit more complicated to install, for that reason *they are not reported as dependencies and are only required when they are about to be used*.
