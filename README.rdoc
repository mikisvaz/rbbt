This is the meta-package for the Rbbt. It produces a gem that installs rbbt-util, rbbt-rest, and rbbt-entities.

I'm working on a documentation page. It is still very incomplete: http://mikisvaz.github.io/rbbt


= Install

To use the Rbbt framework you will need ruby (1.9 preferred), tokyocabinet, and the rbbt-gems.
The installation of these things for a sysadmin should be straight forward. The
following steps cover the complete instalation of the system from a user
account. Due to problems with some of our requirements, both sysadmins and
users should cover steps 3.pre1 and 3.pre2. Steps 5 and 6 should be implemented by
each user.
...
2 - install tokyocabinet (Instructions for user install follow. For system level, make sure we have dev package)
...
3.bis - Or install github repos and make ruby use them
    ...
    alias drbbt="env RBBT_LOG=0 rbbt $(for d in $HOME/git/rbbt-*;do echo -n "-I$d/lib ";done)"
